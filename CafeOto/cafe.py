...
def submit():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        result="SELECT CEmail FROM customer WHERE CEmail=%s"
        cursor.execute(result,(customer,))
        print("if ten önce")
        if result: 
            sorgu= "Select CustomerId From customer Where Cemail=%s"
            cursor.execute(sorgu,(customer,))
            CId = cursor.fetchone()
            CId1=CId["CustomerId"]
            sorgu = "INSERT INTO feedback(CID,rate,FBack) VALUES (%s, %s, %s)"
            cursor.execute(sorgu,(CId1,rating,feedback))
            print("if içi")
        print("if bitti")    
        mysql.connection.commit()
        cursor.close()
        ...
        if customer == '':
            return render_template('index.html', message= "Sadece Müşterilerimiz yorum yapabilir!")
...
def total():
    if request.method == "POST":    
        ...
        sorgu= "Select UnitPrice From orderdetail Where ProductID = %s"
        cursor.execute(sorgu,(total,))
